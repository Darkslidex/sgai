"""
Servicio unificado de precios — estrategia híbrida 3 niveles (ADR-004).

Prioridad: Manual (1.0) > SEPA (0.8) > Scraping (0.6) > Último conocido (fallback)
"""
...
_MAX_MANUAL_DAYS = 7    # Precio manual válido por 7 días
...
class PriceService:
    ...
    Orden de resolución:
    1. ManualPriceAdapter (visto en góndola — máxima confianza)
    2. SEPAPriceAdapter (API gobierno argentino — confianza alta)
    3. ScrapingPriceAdapter (scraping web — confianza media)
    4. Último precio conocido en DB (fallback con warning de antigüedad)
    """
    ...
    async def get_best_price(self, ingredient_id: int) -> MarketPrice | None:
        ...
        # 3. Scraping (confianza media, circuit breaker)
        price = await self._scraping.get_price(ingredient_id)
        if price:
            logger.debug(
                "Precio scraping usado para ingredient_id=%d: ARS=%.2f",
                ingredient_id, price.price_ars,
            )
            return price

        # 4. Último precio conocido (fallback)
    ...
    async def detect_price_anomaly(self, ingredient_id: int, new_price: float) -> bool:
        ...
        Argentina tiene inflación alta, pero un salto de >50% en un día es sospechoso.

        Returns:
            True si el precio es anómalo (requiere confirmación del usuario).
        """
        history = await self._repo.get_price_history(ingredient_id, days=30)
        if len(history) < _ANOMALY_MIN_HISTORY:
            return False  # Sin historial suficiente para detectar anomalías
    ...
    async def get_prices_for_plan(self, ingredient_ids: list[int]) -> dict[int, MarketPrice]:
        ...
        Returns:
            Dict de ingredient_id → MarketPrice (solo incluye ingredientes con precio).
        """
    ...
    def _is_recent(self, price: MarketPrice, max_days: int) -> bool:
        """Retorna True si el precio fue registrado dentro de los últimos max_days días."""
        from datetime import date, timedelta
        cutoff = date.today() - timedelta(days=max_days)
        return price.date >= cutoff

    async def _get_last_known_price(self, ingredient_id: int) -> MarketPrice | None:
        """Retorna el último precio conocido independientemente de la fuente."""
