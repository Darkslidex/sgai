...
async def receive_receipt(
    ...
    """Procesa un ticket de compra.

    Para cada ítem: busca el ingrediente por nombre (ILIKE), registra el precio
    con source='receipt' y confidence=1.0. Los ítems sin match se reportan
    como skipped para que Ana informe al usuario.
    """
    ...
    for item in body.items:
        matches = await ing_repo.search_ingredients(item.product_name)
        if not matches:
        ...
        price = MarketPrice(
            ...
            confidence=1.0,
            date=body.purchase_date,
            created_at=datetime.utcnow(),
        )
        await market_repo.add_price(price)
        registered += 1

    message = f"{registered} precio(s) registrado(s)"
    if skipped:
        message += f", {len(skipped)} ítem(s) sin match en la DB."
    else:
        message += "."
